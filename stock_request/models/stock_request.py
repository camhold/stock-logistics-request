...
class StockRequest(models.Model):
    ...
    route_id = fields.Many2one(
        'stock.route',
        related='order_id.route_id',  # Ruta de la cabecera
        readonly=False,
        store=True,
        help="Route for this stock request, related to the header"
    )
    ...
    def _compute_stock_available(self):
        for record in self:
            if record.product_id and record.route_id:
                # Obtener la ruta
                route = record.route_id
                # Obtener el almacén proveedor asociado a la ruta
                supplier_warehouse = route.supplier_wh_id

                # Verificar que el almacén proveedor exista
                if supplier_warehouse:
                    # Usar el método _gather para obtener el stock cuantificado en el almacén de entrega
                    quants = self.env['stock.quant']._gather(record.product_id, supplier_warehouse.lot_stock_id)
                    record.stock_available = sum(quant.quantity for quant in quants)
                else:
                    record.stock_available = 0.0

                # Logging para depuración
                _logger.info(f"Producto: {record.product_id.name}, Supplier Warehouse: {supplier_warehouse.name if supplier_warehouse else 'N/A'}, Stock Available: {record.stock_available}")
            else:
                record.stock_available = 0.0
